    ...
    <div class="row justify-content-center">
        <div class="col-12 col-md-6">
                   ...
                   <div class="container ">
                    <div class="row justify-content-center">
                    @if(Auth::user()->is_revisor)
                        <a href="{{route('revisor.index')}}" class="btn btn-primary">{{__('ui.backToDash')}}</a>
                    @else
                        <a href="{{route('homepage')}}" class="btn btn-primary">home</a>
                    @endif
                    <a class="btn text-center" href="{{route('category',['category'=>$article->category])}}">{{__('ui.categoria')}}: {{ trans('categories.' . $article->category->name) }}</a>
